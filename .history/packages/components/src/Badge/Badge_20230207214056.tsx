...
const Badge = React.forwardRef((props: BadgeProps, ref) => {
  const { color, count, dot, offset, overflowCount = 99, showZero = false, title, className, style, children } = props
  ...
  React.Children.map(children, (c) => {
    return React.cloneElement(
      ...
      <div className={'son'} style={{ backgroundColor: 'red', width: '100px', height: '50px' }}></div>,
    )
  })
