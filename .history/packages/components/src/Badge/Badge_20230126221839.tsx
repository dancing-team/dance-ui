...
const Badge: React.FunctionComponent<React.PropsWithChildren<BadgeProps>> = React.forwardRef(function BadgeInner(
  ...
  let overflowCount1: number = 99
  overflowCount1 = overflowCount ? overflowCount : 99
  return (
    <div
      className={
        type === 'unstyle'
      ...
      {dot === undefined && count && count > 0 && count <= overflowCount1
        ? count
        : count && count > overflowCount1
        ? overflowCount1.toString() + '+'
        : ''}
    </div>
  )
})
